...
def alarmsystem(Hours2, Minutes2): #Main alarm loop
    ...
    if soundisplaying == True: #Checks if sound is playing because if it is and you stop sound it will break
        mixer.music.stop()
        soundisplaying = False
    if timeofday == "Bedtime" and background != [] and settings0 == "backgroundsound = on": #Insures that the background sound won't play if it doesn't exist
        soundisplaying = True
        playsound(random.choice(background))
    elif soundsorno == "Y" or soundsorno == "Yes" or soundsorno == "y" or soundsorno == "yes" and background != [] and settings0 == "backgroundsound = on/n":
        playsound(random.choice(background))
        soundisplaying = True
